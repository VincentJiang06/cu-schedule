...
| `programs.json` | `ProgramBundle`(见 programs.ts) | 223KB | `?v=` 不可变 |
...
新增用户可见中文必须包 `t()`;生成链 `npm run i18n:extract` → `npm run i18n:gen`
(繁=OpenCC s2hk,英=DeepSeek)。**`.md` 配置(configMd)的节标题是解析协议 key,
恒简体不得 localize**(写出侧本地化会破坏回导入)。
...
已清偿(2026-07-15,留档防复发):`programs.ts` 的 `?v=`(前端已接,nginx 的
no-cache carve-out 已并回 immutable 并线上验证);`index.json` 启动双请求
(data.ts 按学年 memoize,`fetchYearIndex`)。
