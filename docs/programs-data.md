...
```
public/data/programs.json          ← 前端 fetch 的文件（UG，2023/2024/2025，245 个专业，原始 ~1.8MB / gzip ~223KB）
...
tt_dsp_acad_prog.aspx (CUHK)
  → scrape_programs.py        data/raw/programs/<入学年>/<Faculty>/<slug>.{json, html.gz}   原始（含完整源页 .html.gz）【只读】
  → parse_programs.py         data/programs/<入学年>/…/*.json                              解析产物
  → build_bundles.mts         data/programs/programs.json → 镜像 public/data/              前端包（与课程包同一个脚本）
```

重新生成前端包：`npm run data:build`（课程包 + programs 包一起重建并镜像）。改过
`parse_programs.py` 或刷新方案数据后，必跑 `npm run data:audit-programs`（机器门
P1–P6，零丢课/零学分谎报审计）再收工。
